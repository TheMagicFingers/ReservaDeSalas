...
void relatorio(){
    ...
    printf("Relatorio gerado em: Relatorio/relatorio.txt\n");
    FILE *arq, *arqR;
    arq = fopen("db/dbSala.bin", "r");
    ...
    if(arq != NULL){
        ...
        fprintf(arqR, "ID\tBLOCO/NUM\tTIPO\n");
        fprintf(arqR, "\n");
    ...
    if (arq != NULL) {
        Reserva reserva;
        fprintf(arqR, "\n-----------RESERVAS REALIZADAS-----------\n\n");
        fprintf(arqR, "ID\tSala\tData da reserva\t\tProfessor\n");
        while (fread(&reserva, sizeof(reserva), 1, arq)) {
